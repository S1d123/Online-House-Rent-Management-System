...
public class changeCustomerDetails{
	...
	public changeCustomerDetails() {
		...
		SAVEBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String sql="select * from viewtenantdetails where Username = '"+ uname +"' ";
					
					ResultSet rs=sta.executeQuery(sql);
					if (rs.next()) {
						if (rs.getString("Password").equals(oldpwd)) {
							if (newpwd.equals(cnfpwd)) {
								String sql1="update viewtenantdetails set Password='"+ newpwd+"' where Username='"+uname+"' ";
		...
		SAVEBTN.setFont(new Font("Eras Medium ITC", Font.BOLD, 27));
		SAVEBTN.setBackground(new Color(51, 0, 0));
		SAVEBTN.setBounds(50, 511, 192, 51);
